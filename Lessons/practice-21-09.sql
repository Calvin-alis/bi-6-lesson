...
SELECT CONCAT('OLEKSANDR', 'NAME', 'NAME') AS 'Full name';
...
FROM 
employees.salaries
WHERE emp_no = 10050 OR emp_no = 10051;

SELECT *
FROM 
employees.employees
WHERE (first_name = last_name) AND gender = 'F';
...
FROM 
employees
WHERE first_name NOT LIKE 'a___';
...
SELECT 
emp_no, sum(emp_no), avg(salary), (SELECT AVG(salary) FROM salaries) 
FROM salaries
GROUP BY emp_no;
...
SELECT 
emp_no, sum(salary), ABS(avg(salary) -  (SELECT AVG(salary) FROM salaries) )
FROM salaries 
GROUP BY emp_no
...
INNER JOIN employees.salaries AS es ON(ee.emp_no = es.emp_no AND ee.hire_date  = es.from_date);


SELECT 
*
FROM employees AS ee
LEFT JOIN employees.salaries AS es ON(ee.emp_no = es.emp_no AND ee.hire_date  = es.from_date);

SELECT emp_no FROM employees WHERE emp_no BETWEEN 10010 AND 10020
UNION
SELECT first_name FROM employees WHERE emp_no BETWEEN 11010 AND 11020
 ;
...
SELECT emp_no, SUM(salary) , (SELECT SUM(salary) FROM salaries )
FROM 
salaries
...
SELECT *
FROM (SELECT * FROM salaries) AS es;
# 
-- 
/*
*/
