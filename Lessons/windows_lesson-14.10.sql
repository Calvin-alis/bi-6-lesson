...
OVER (ORDER BY emp_no  ASC),  -- Over  ключове слово для створення вікна, в середині пропусуємо order by, щоб корректно показувати інформацію
ee.*
...
SELECT name, score,
    ...
    (ORDER BY score) -- Сортування по score
    row_num,
...
/*
	Ранжирование даних 
    Dense rank
    Rank  - Оператор RANK схожий на ROW_NUMBER, але присвоює однакові номери рядкам з однаковими значеннями, а "зайві" номери пропускає
*/
